#!/usr/bin/env python3
"""MetaBlooms web coding usage quality (WCUQ) score status surface.

Reads validation and gate receipts and writes a v2 WCUQ status object to
runtime/state/WCUQ_STATUS.json and a plain-text display surface to
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Write WCUQ v2 status surface")
    ap.add_argument("--validation", default="")
    ap.add_argument("--gate", default="")
    ap.add_argument("--extra", default="")
    ap.add_argument("--out-txt", default="runtime/state/WCUQ_STATUS.txt")
    ap.add_argument("--out-json", default="runtime/state/WCUQ_STATUS.json")
    args = ap.parse_args()
    root = Path.cwd()
